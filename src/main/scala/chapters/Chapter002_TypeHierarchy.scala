...
class Chapter002_TypeHierarchy
	...
	if (printSubtitle("In Scala, all values have a type, including numerical values and functions. The diagram below illustrates a subset of the type hierarchy.", false))
		println(ScalaTypeHierarchy)

	if (printSubtitle("Scala Type Hierarchy", false)) {
	...
	if (printSubtitle("Type Casting", true)) {
